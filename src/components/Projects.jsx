...
const ProjectCard = ({ project, aosDelay }) => (
  <div 
    ...
    className={`relative rounded-2xl p-[1px] group transition-all duration-500 ${
      project.isFlagship 
        ? 'bg-gradient-to-br from-violet-500/50 via-white/10 to-violet-500/30 hover:from-violet-500 hover:via-violet-400/30 hover:to-violet-500/60' 
  ...
  >
    <div className={`rounded-2xl p-6 md:p-8 h-full backdrop-blur-md transition-all duration-500 ${
      ...
      {/* Badge */}
      {project.badge && (
        <span className="inline-block text-xs font-bold tracking-widest uppercase text-violet-400 bg-violet-500/10 px-3 py-1 rounded-full border border-violet-500/20 mb-4">
          {project.badge}
        </span>
      )}
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        {project.techTags.map((tag) => (
          <span 
            key={tag}
            className="px-3 py-1 text-xs font-bold text-white/70 bg-white/5 rounded-full border border-white/10 hover:bg-violet-500/20 hover:border-violet-500/30 hover:text-red-300 transition-all duration-300 cursor-default"
          >
            {tag}
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {project.links.demo !== undefined && (
          <a 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all duration-300 ${
              project.links.demo 
                ? 'bg-[#7C3AED] text-white hover:bg-violet-600 hover:shadow-[0_0_20px_rgba(124,58,237,0.4)]' 
        ...
        {project.links.frontendDemo && (
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 px-4 py-2 rounded-full bg-[#7C3AED] text-white text-sm font-semibold hover:bg-violet-600 hover:shadow-[0_0_20px_rgba(124,58,237,0.4)] transition-all duration-300"
          >
            <ExternalLinkIcon />
            Frontend Demo
          </a>
